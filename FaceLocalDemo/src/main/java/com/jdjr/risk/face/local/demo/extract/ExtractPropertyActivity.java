package com.jdjr.risk.face.local.demo.extract;
...
import com.jdjr.risk.face.local.demo.camera.MagicCameraManager;
...
public class ExtractPropertyActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_extract_property);
...
//        05-13 02:55:49.054 12309-12309/com.jdjr.risk.face.local.demoooooooo D/FaceLocalResolution: .......... heightPixels = 720
        ...
        if (widthPixels == 1208 && heightPixels == 720) {
            ...
            final View previewRGB = findViewById(R.id.camera_preview_rgb);
    ...
    private MagicCameraView RGBTextureView;
//    private TextureView RGBTextureView;
    ...
    private static Bitmap decodeYUV(byte[] yuvFrame) {
        final YuvImage yuvImage = new YuvImage(yuvFrame, ImageFormat.NV21, CameraSettings.getCameraWidth(), CameraSettings.getCameraHeight(), null);
        final ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        yuvImage.compressToJpeg(new Rect(0, 0, CameraSettings.getCameraWidth(), CameraSettings.getCameraHeight()), 70, outputStream);
        final byte[] jpgFrame = outputStream.toByteArray();
        final Bitmap bitmap = BitmapFactory.decodeByteArray(jpgFrame, 0, jpgFrame.length);
        return bitmap;
    ...
    private final FaceFrameManager.BasePropertyCallback mBasePropertyCallback = new FaceFrameManager.BasePropertyCallback() {
        ...
        public void onBasePropertyResult(List<DetectFaceInfo> faceBoxList) {
            if (faceBoxList != null && faceBoxList.size() > 0) {
                ExtractPropertyActivity.this.onFaceResult(faceBoxList);
            } else {
                onFaceLost();
            }
...
//        Log.d("FaceLocalVerifyDebug", "......... appendVerifyResult getVerifyResult userId = " + userId);
    ...
    public void makeWhiten(View view) {
        
        if (whiten) {
            MagicCameraManager.getInstance().setFilter(MagicFilterType.SKINWHITEN);
        } else {
            MagicCameraManager.getInstance().setFilter(MagicFilterType.NONE);
        }
    ...
    public void makeSmooth(View view) {
        final String smoothLevelStr = (String) view.getTag();
        final int smoothLevel = Integer.parseInt(smoothLevelStr);
        Log.d("FaceLocalMagic", "==============smoothLevel = " + smoothLevel);
        MagicCameraManager.getInstance().setBeautyLevel(smoothLevel);
    ...
    public void takePicture(View view) {
        final String path = getMagicPicturePath();
        // final File file = getOutputMediaFile();
        // MagicCameraManager.getInstance().takePicture(path);
        
        
//        FaceCameraHelper.getInstance().takePicture(path);
        
    }
    
    
    private String getMagicPicturePath() {
        final File dir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES), "FaceMagicPicture");
        dir.mkdirs();
        final String name = "FaceMagicPicture" + "_" + new SimpleDateFormat("yyyy-MM-dd-HH:mm:ss:SSS").format(new Date()) + ".jpg";
        final String path = dir + File.separator + name;
